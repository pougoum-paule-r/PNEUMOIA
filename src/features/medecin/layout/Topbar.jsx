...
import { Menu, Search, Bell, MessageCircle, User, Settings, LogOut, UserCircle, ChevronDown } from 'lucide-react';
...
export default function Topbar({ sidebarOpen, setSidebarOpen, pageTitle }) {
  ...
  return (
    <>
      <header className="sticky top-0 z-20 bg-white border-b border-slate-200 shadow-sm w-full">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Titre de la page */}
            <div>
              <h2 className="text-xs font-black uppercase tracking-[0.5em] text-slate-300">
                {pageTitle}
              </h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-slate-50 rounded-2xl border border-slate-100 min-w-[280px] transition-all focus-within:border-blue-300 focus-within:shadow-sm">
            ...
            <button
              ...
              className="md:hidden p-2 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-all"
            >
            ...
            <Link 
              ...
              className="relative p-2 rounded-xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-all group"
            >
            ...
            <div className="relative" ref={userMenuRef}>
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-slate-100 transition-all group"
              >
                ...
                <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="p-2">
                    {userMenuItems.map((item, idx) => (
                      item.divider ? (
                        <div key={idx} className="h-px bg-slate-100 my-2"></div>
                      ) : (
                        <Link
                          key={idx}
                          to={item.path}
                          ...
                          className={`
                            ...
                            ${item.className || 'text-slate-600 hover:bg-slate-50'}
                          `}
                        >
                          <item.icon className={`w-4 h-4 ${item.className ? 'text-red-500' : 'text-blue-500'}`} />
                  ...
                  <div className="p-3 bg-slate-50 border-t border-slate-100">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="text-center py-1">
                        <p className="text-[8px] font-black text-slate-400 uppercase">Patients</p>
                        <p className="text-xs font-bold text-slate-900">234</p>
                      </div>
                      <div className="text-center py-1">
                        <p className="text-[8px] font-black text-slate-400 uppercase">Consultations</p>
                        <p className="text-xs font-bold text-slate-900">1,289</p>
                      </div>
                    </div>
                  </div>
                </div>
