      ...
      <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-800">{patientInfo.civilite} {patientInfo.prenom} {patientInfo.nom}</p>
              <p className="text-xs text-slate-500">{patientInfo.age} • {patientInfo.telephone || 'Tél. non renseigné'}</p>
            </div>
          </div>
        ...
        return (
          <div key={step.number} className="flex-1 relative">
            <div className="flex flex-col items-center">
              ...
              <p className={`text-[10px] font-medium mt-1.5 ${isActive ? 'text-blue-600' : isCompleted ? 'text-emerald-600' : 'text-slate-400'}`}>
                {step.label}
              </p>
            </div>
            {index < steps.length - 1 && (
              <div className={`absolute top-5 left-[calc(50%+20px)] w-[calc(100%-40px)] h-0.5 transition-all duration-300 ${isCompleted ? 'bg-emerald-600' : 'bg-slate-200'}`} />
            )}
          </div>
        );
      })}
...
const FormCard = ({ title, icon, children, className = "" }) => (
  <div className={`bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all ${className}`}>
  ...
  <div className="mb-3">
    ...
    <div className="relative">
      ...
      <input
        ...
        disabled={disabled}
        className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${Icon ? 'pl-9' : ''} ${disabled ? 'bg-slate-50 text-slate-500' : ''}`}
      />
    </div>
  </div>
);
  ...
  <div className="mb-3">
    ...
    <div className="relative">
      ...
      <select
        value={value || ''}
        onChange={onChange}
        className={`w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white ${Icon ? 'pl-9' : ''}`}
      >
...
const CheckboxField = ({ label, checked, onChange, description }) => (
  <label className="flex items-start gap-2 cursor-pointer p-1 rounded hover:bg-slate-50 transition-colors">
    <input
      type="checkbox"
      checked={checked}
...
export default function Consultation() {
  ...
  const renderStepPatient = () => (
    <div className="space-y-4">
      <div className="flex gap-3">
        <button onClick={() => setShowSearch(false)} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${!showSearch ? 'bg-blue-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
          <UserPlus className="w-4 h-4 inline mr-1.5" /> Nouveau patient
        </button>
        <button onClick={() => setShowSearch(true)} className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${showSearch ? 'bg-blue-600 text-white shadow-sm' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
      ...
      {showSearch && (
        <div className="bg-white rounded-lg border border-slate-200 p-4">
          ...
          {searchResults.length > 0 && searchResults.map(p => (
            <div key={p.id} className="flex justify-between items-center p-2 bg-slate-50 rounded-lg mt-2">
              <div><p className="font-medium text-sm">{p.nom} {p.prenom}</p><p className="text-xs text-slate-500">{p.telephone}</p></div>
  ...
  const renderStepAntecedents = () => (
    <div className="space-y-4">
      <FormCard title="Mode de vie" icon={<Activity className="w-4 h-4" />}>
        <div className="mb-3">
          <label className="block text-xs font-medium text-slate-700 mb-2">Tabagisme</label>
  ...
  const renderStepDiagnostic = () => (
    <div className="space-y-4">
      {/* Bannière risque */}
      <div className={`bg-gradient-to-r from-${getRiskColor(aiResult.riskLevel)}-500 to-${getRiskColor(aiResult.riskLevel)}-600 rounded-xl p-4 text-white`}>
        <div className="flex items-center gap-2">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-bold text-sm mb-3 flex items-center gap-2"><Target className="w-4 h-4 text-blue-600" /> Diagnostic principal</h3>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-bold text-sm mb-3 flex items-center gap-2"><LineChart className="w-4 h-4 text-emerald-600" /> Diagnostics différentiels</h3>
      ...
      {aiResult.criteria.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-bold text-sm mb-2 flex items-center gap-2"><CheckCircle className="w-4 h-4 text-emerald-600" /> Critères validés ({aiResult.criteria.length})</h3>
      ...
      {aiResult.examensRecommandes.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <h3 className="font-bold text-sm mb-2 flex items-center gap-2"><Microscope className="w-4 h-4 text-purple-600" /> Examens recommandés</h3>
          <div className="flex flex-wrap gap-1.5">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4">
        <h3 className="font-bold text-sm text-blue-900 mb-2 flex items-center gap-2"><Zap className="w-4 h-4" /> Recommandations</h3>
        <div className="space-y-1.5">
          {aiResult.recommendations.map((rec, i) => (
            <div key={i} className="flex items-start gap-1.5 p-1.5 bg-white rounded-lg text-xs">
      ...
      <FormCard title="Prescription médicale" icon={<Pill className="w-4 h-4" />}>
        ...
        <div className="mt-3 pt-3 border-t border-slate-100">
          <div className="flex items-start gap-3">
            ...
            <div>
              <label htmlFor="shareCase" className="text-sm font-medium text-slate-700 cursor-pointer">Partager ce cas avec la communauté médicale</label>
              <p className="text-xs text-slate-400">Les données personnelles seront anonymisées (nom, date, lieu). Ce cas aidera à former l'IA pour les générations futures.</p>
            </div>
          </div>
