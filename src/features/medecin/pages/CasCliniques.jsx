  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="px-6 pt-5 pb-2">
        ...
        <h3 className="text-lg font-bold text-slate-800 leading-tight">{cas.titre}</h3>
        <p className="text-sm text-slate-500 mt-2 line-clamp-2">{cas.description}</p>
      </div>
      ...
      <div className="px-6 py-3 bg-slate-50 border-y border-slate-100">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-600">IA Confidence</span>
          </div>
      ...
      <div className="px-6 py-4">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-slate-800">{cas.auteur}</p>
              <p className="text-xs text-slate-400">{cas.hopital}</p>
            </div>
          </div>
          ...
          {isExpanded && (
            ...
            >
              <div className="bg-slate-50 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-slate-800 mb-2">Analyse détaillée</h4>
                <p className="text-sm text-slate-600 leading-relaxed">{cas.analyse}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  {cas.tags?.map(tag => (
                    <span key={tag} className="text-xs px-2 py-1 rounded-full bg-white border border-slate-200 text-slate-600">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-slate-800">Cas cliniques</h1>
          </div>
          <p className="text-slate-500">Analysez, votez et contribuez aux cas cliniques partagés par la communauté médicale CEMAC.</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-semibold text-slate-800">Top Contributeurs</h2>
          </div>
            ...
            {topContributors.map((contrib, idx) => (
              <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                ...
                <div className="flex-1">
                  <p className="font-medium text-slate-800 text-sm">{contrib.nom}</p>
                  <p className="text-xs text-slate-400">{contrib.hopital}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-bold text-blue-600">{contrib.score}%</p>
                  <p className="text-xs text-slate-400">{contrib.casPartages} cas</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-5">
          {casFiltres.length === 0 ? (
            <div className="bg-white rounded-xl p-12 text-center border border-slate-200">
              ...
              <p className="text-slate-500">Aucun cas clinique ne correspond à vos critères.</p>
            </div>
          ) : (
            casFiltres.map(c => (
        ...
        {/* CTA - PARTAGER UN CAS */}
        <div className="mt-10 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button onClick={() => navigate('/medecin/consultation')} className="px-6 py-3 bg-white text-blue-600 rounded-xl font-semibold shadow-md hover:shadow-lg transition-all flex items-center gap-2">
