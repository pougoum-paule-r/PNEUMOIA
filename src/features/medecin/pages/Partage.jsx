...
const GlassCard = ({ children, className = "" }) => (
  <div className={`relative bg-white rounded-2xl border border-slate-200 shadow-md ${className}`}>
    {children}
  </div>
);
  ...
  return (
    <div className="flex justify-center items-center gap-2 mt-6">
      <button
        ...
        className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
      >
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-5 mb-10">
          {stats.map((stat, idx) => (
            <div key={stat.label} className="relative">
              <div className="relative bg-white rounded-2xl p-5 shadow-sm border border-slate-200">
        ...
        <AnimatePresence mode="wait">
            ...
            >
              {demandesPaginated.length === 0 ? (
                ...
                demandesPaginated.map((demande) => (
                  <div key={demande.id} className="relative bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-bold text-slate-800 text-lg">{demande.patient}</span>
                              <Badge variant="info">
                            ...
                            <p className="text-sm text-slate-500">{demande.patientAge} ans · {demande.patientSexe === 'M' ? 'Homme' : 'Femme'}</p>
                          </div>
                        </div>
                      ...
                      <div>
                        <p className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-3">Médecins demandeurs</p>
                        <div className="space-y-3">
                          {demande.medecins.map((medecin, medIdx) => (
                            <div
                              key={medecin.id}
                              className="flex flex-wrap items-center justify-between gap-3 p-3 bg-slate-50 rounded-xl"
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-semibold text-slate-800 text-sm">{medecin.nom}</p>
                                  <p className="text-xs text-slate-400">{medecin.hopital}</p>
                                </div>
                              </div>
          ...
          {activeTab === 'dossiers' && (
            ...
            >
              {dossiersPaginated.map((dossier) => (
                <div key={dossier.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm">
                  <div className="p-6">
                    {/* En-tête patient */}
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-4 pb-3 border-b border-slate-100">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-slate-800 text-lg">{dossier.patient}</div>
                          <div className="text-xs text-slate-400 font-mono">{dossier.idPatient}</div>
                        </div>
                      </div>
                      <div className="text-xs text-slate-400">Partagé le {dossier.date}</div>
                    </div>

                    {/* Accès partagés */}
                    <div>
                      <p className="text-xs font-medium text-slate-400 uppercase tracking-wide mb-3">Accès partagés</p>
                      <div className="space-y-2">
                        {dossier.medecins.map((medecin, medIdx) => (
                          <div
                            key={medIdx}
                            className="flex flex-wrap items-center justify-between gap-3 p-2.5 hover:bg-slate-50 rounded-lg transition-colors"
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-medium text-slate-700 text-sm">{medecin.nom}</p>
                                <p className="text-xs text-slate-400">{medecin.hopital}</p>
                              </div>
                            </div>
                            <button
          ...
          {activeTab === 'communaute' && (
            ...
            >
              {casCommunautaires.map((cas) => (
                <div key={cas.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="p-6">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex gap-1.5 mb-3 flex-wrap">
                          {cas.tags.map(tag => (
                            <span key={tag} className="text-xs px-2 py-1 rounded-full bg-slate-100 text-slate-600 font-medium">
                              {tag}
                            </span>
                          ))}
                        </div>
                        <h3 className="text-xl font-bold text-slate-800 mb-2 leading-tight">{cas.titre}</h3>
                        <div className="flex items-center gap-2 text-sm text-slate-500">
                          <div className="w-6 h-6 rounded-full bg-blue-600 flex items-center justify-center text-white text-[10px] font-bold">
                            {cas.auteur.charAt(0)}
                          </div>
                          <span className="font-medium text-slate-700">{cas.auteur}</span>
                          <span className="w-1 h-1 rounded-full bg-slate-400" />
                          <span>{cas.hopital}</span>
                        </div>
                        <p className="text-xs text-slate-400 mt-2">{cas.date}</p>
                      </div>
                      <button
                        onClick={() => likeCas(cas.id)}
                        className="flex flex-col items-center px-3 py-2 rounded-xl hover:bg-red-50 transition-colors"
                      >
                        <Heart className={`w-6 h-6 ${cas.liked ? 'fill-red-500 text-red-500' : 'text-slate-400'}`} />
                        <span className="text-xs font-bold text-slate-600 mt-1">{cas.likes}</span>
                      </button>
                    </div>
          ...
          {activeTab === 'groupes' && (
            ...
            >
              {groupes.map((groupe) => (
                <div key={groupe.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="font-bold text-slate-800 text-xl">{groupe.nom}</h3>
                          <p className="text-xs text-slate-500">{groupe.membres} membres actifs</p>
                        </div>
                      </div>
                      {groupe.nouveauxCas && (
                    ...
                    <div className="mt-4">
                      ...
                      {groupe.description && (
                        <p className="text-sm text-slate-600">{groupe.description}</p>
                      )}
                    </div>

                    <div className="mt-5 flex gap-3 pt-4 border-t border-slate-100">
                      <button className="flex-1 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-semibold hover:bg-slate-200 transition-all flex items-center justify-center gap-2">
              ...
              <div className="flex justify-center col-span-full mt-4">
                <button className="px-8 py-3.5 bg-slate-800 text-white rounded-xl font-semibold shadow-md hover:bg-slate-700 transition-all flex items-center gap-2">
