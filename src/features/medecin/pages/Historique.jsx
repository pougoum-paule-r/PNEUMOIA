...
export default function ConsultationHistory() {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      completed: { label: 'Terminée', icon: CheckCircle, className: 'bg-emerald-50 text-emerald-700' },
      cancelled: { label: 'Annulée', icon: XCircle, className: 'bg-red-50 text-red-700' },
      pending: { label: 'En attente', icon: AlertCircle, className: 'bg-amber-50 text-amber-700' }
    };
        ...
        return (
          ...
          >
            <div className="p-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-slate-900">{consult.patient.name}</h3>
                    <p className="text-xs text-slate-400">{consult.patient.age} ans</p>
                  </div>
                </div>
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                <p className="text-xs text-slate-400 mb-1">Pathologie</p>
                <p className="text-sm font-medium text-slate-800">{consult.pathology}</p>
              </div>
            ...
            <div className="px-4 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
              ...
              <button className="text-blue-600 text-sm font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
                Voir détails
    ...
    <div className="overflow-x-auto">
      <table className="w-full">
        <thead className="bg-slate-50 border-b border-slate-200">
          <tr>
            <th className="text-left py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Patient</th>
            <th className="text-left py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Date & Heure</th>
            <th className="text-left py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Pathologie</th>
            <th className="text-left py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Durée</th>
            <th className="text-left py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Statut</th>
            <th className="text-right py-4 px-5 text-xs font-semibold text-slate-500 uppercase tracking-wider">Action</th>
          </tr>
        </thead>
              ...
              >
                <td className="py-4 px-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-slate-900">{consult.patient.name}</p>
                      <p className="text-xs text-slate-400">{consult.patient.age} ans</p>
                    </div>
                  </div>
                </td>
                <td className="py-4 px-5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-600">{formatDate(consult.date)}</span>
                    <Clock className="w-3.5 h-3.5 text-slate-400 ml-2" />
                    <span className="text-sm text-slate-600">{consult.time}</span>
                  </div>
                </td>
                <td className="py-4 px-5">
                  <p className="text-sm text-slate-900">{consult.pathology}</p>
                </td>
                <td className="py-4 px-5">
                  <span className="text-sm text-slate-600">{consult.duration || '-'}</span>
                </td>
                <td className="py-4 px-5">
  ...
  const ConsultationModal = () => {
    ...
    return (
      <>
        ...
        />
        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-white rounded-2xl shadow-2xl z-50 overflow-hidden max-h-[90vh] overflow-y-auto">
          {/* En-tête du modal */}
          <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-indigo-50 sticky top-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Stethoscope className="w-6 h-6 text-blue-600" />
                <h2 className="text-xl font-bold text-slate-900">Détails de la consultation</h2>
              </div>
              <button
                onClick={() => setSelectedConsultation(null)}
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
              ...
              <div>
                <p className="text-lg font-bold text-slate-900">{selectedConsultation.patient.name}</p>
                <p className="text-sm text-slate-500">{selectedConsultation.patient.age} ans</p>
            ...
            <div className="p-4 bg-white border border-slate-200 rounded-xl">
              <h3 className="text-sm font-semibold text-slate-900 mb-1">Pathologie</h3>
              <p className="text-sm text-slate-600">{selectedConsultation.pathology}</p>
            </div>

            {/* Notes médicales */}
            {selectedConsultation.notes && (
              <div className="p-4 bg-slate-50 rounded-xl">
                <h3 className="text-sm font-semibold text-slate-900 mb-2">Notes médicales</h3>
                <p className="text-sm text-slate-600">{selectedConsultation.notes}</p>
              </div>
            )}
            ...
            {selectedConsultation.prescription && (
              <div className="p-4 bg-blue-50 rounded-xl">
                <h3 className="text-sm font-semibold text-slate-900 mb-2">Prescription</h3>
                <p className="text-sm text-slate-600">{selectedConsultation.prescription}</p>
              </div>
            )}
            ...
            {selectedConsultation.nextAppointment && (
              <div className="flex items-center justify-between p-4 border border-slate-200 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-500">Prochain rendez-vous</p>
          ...
          <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 sticky bottom-0">
            <button
              onClick={() => setSelectedConsultation(null)}
              className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-white rounded-lg transition-colors"
            >
              Fermer
            </button>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Historique des consultations</h1>
          <p className="text-sm text-slate-500 mt-1">Consultez l'historique complet de vos consultations en présentiel</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <div className="flex gap-2">
          <select
            ...
            className="px-4 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
          >
          ...
          <div className="flex border border-slate-200 rounded-xl overflow-hidden">
            <button
              ...
              className={`p-2 px-3 transition-all ${viewMode === 'cards' ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
            >
              <LayoutGrid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode('table')}
              className={`p-2 px-3 transition-all ${viewMode === 'table' ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
            >
