    ...
    <header className={`fixed top-0 left-0 right-0 lg:left-64 z-40 flex items-center justify-between gap-4 px-4 sm:px-6 py-3 border-b shadow-sm ${darkMode ? "bg-gray-900 border-gray-800 text-white" : "bg-white border-gray-100 text-gray-900"}`}>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className={`text-[11px] hidden sm:block ${darkMode ? "text-gray-500" : "text-gray-400"}`}>Super Administration - PneumoIA CEMAC</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        <div className={`hidden sm:flex items-center gap-2 rounded-xl px-3 py-2 text-sm border transition-all duration-150 ${darkMode ? "bg-gray-800 border-gray-700 text-gray-300" : "bg-gray-50 border-gray-200 text-gray-500"}`}>
          ...
          <input type="text" placeholder="Rechercher un medecin..." value={searchVal} onChange={(e) => setSearchVal(e.target.value)} className={`bg-transparent outline-none w-44 text-sm placeholder:text-gray-400 ${darkMode ? "text-gray-200" : "text-gray-700"}`} />
        </div>
        ...
        <div className="relative">
          <button onClick={() => setProfileOpen(!profileOpen)} className="w-8 h-8 rounded-full bg-teal-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">SA</button>
          {profileOpen && (
            <div className={`absolute right-0 mt-2 w-56 rounded-xl shadow-lg border p-4 z-50 ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
              <p className={`text-sm mb-4 ${darkMode ? "text-gray-300" : "text-gray-700"}`}>superadmin@gmail.com</p>
