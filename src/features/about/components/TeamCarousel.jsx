    ...
    <section className="py-20 px-4 bg-gray-50">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Equipe de Travail</h2>
        ...
        <div className="relative px-8 md:px-12">
          ...
          {totalPages > 1 && (
            <>
              <button 
                onClick={prev} 
                className="absolute left-0 top-1/2 -translate-y-1/2 p-2 bg-white rounded-full shadow-lg hover:bg-gray-50 transition-all border border-gray-200"
              >
                <ChevronLeft className="w-6 h-6 text-gray-600" />
              </button>
              <button 
                onClick={next} 
                className="absolute right-0 top-1/2 -translate-y-1/2 p-2 bg-white rounded-full shadow-lg hover:bg-gray-50 transition-all border border-gray-200"
              >
          ...
          <div className="flex justify-center gap-2 mt-8">
            {Array.from({ length: totalPages }).map((_, idx) => (
              <button 
                ...
                className={`w-2 h-2 rounded-full transition-all ${currentIndex === idx ? 'w-6 bg-blue-600' : 'bg-gray-300'}`}
              />
            ))}
          </div>
        )}
