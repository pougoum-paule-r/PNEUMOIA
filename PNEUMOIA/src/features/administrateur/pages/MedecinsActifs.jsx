...
const MOCK_MEDECINS = [
  { id:1, initiales:"JD", couleur:"bg-teal-600", nom:"Dr. Jean Dupont", specialite:"Pneumologue", cnom:"CM-2019-0847", etablissement:"H. Général Douala", ville:"Douala", patients:134, consultations:4821, concordance:88, creeLE:"12/06/2024", valideLE:"15/06/2024", statut:"Actif", email:"j.dupont@hgd.cm", telephone:"+237 699 123 456", rangCommunaute:"#7/38", casPartages:"247 cas publiés", activiteRecente:[{texte:"Consultation #247 enregistrée",quand:"Auj. 14:22"},{texte:"Cas #241 partagé sur la communauté",quand:"Hier"},{texte:"Accès accordé à Dr. Martin",quand:"Il y a 3j"},{texte:"3 consultations — Pneumonie",quand:"15 mars"}] },
  { id:2, initiales:"DK", couleur:"bg-blue-600",  nom:"Dr. Kamto",      specialite:"Pneumologue", cnom:"CM-2017-0432", etablissement:"CHU Yaoundé",      ville:"Yaoundé", patients:198, consultations:3201, concordance:94, creeLE:"03/04/2024", valideLE:"05/04/2024", statut:"Actif", email:"d.kamto@chu.cm",  telephone:"+237 677 234 567", rangCommunaute:"#3/38", casPartages:"312 cas publiés", activiteRecente:[{texte:"Consultation #198 enregistrée",quand:"Auj. 09:10"},{texte:"Rapport mensuel soumis",quand:"Il y a 2j"}] },
  { id:3, initiales:"DN", couleur:"bg-purple-600",nom:"Dr. Nkoa",       specialite:"Pneumologue", cnom:"CM-2018-0521", etablissement:"H. Général Douala", ville:"Douala",  patients:176, consultations:2847, concordance:91, creeLE:"14/05/2024", valideLE:"16/05/2024", statut:"Actif", email:"nkoa@hgd.cm",     telephone:"+237 699 345 678", rangCommunaute:"#5/38", casPartages:"189 cas publiés", activiteRecente:[{texte:"Nouveau patient enregistré",quand:"Hier"},{texte:"Cas #235 publié",quand:"Il y a 5j"}] },
  { id:4, initiales:"DB", couleur:"bg-orange-500",nom:"Dr. Barry",      specialite:"Pneumologue", cnom:"CM-2020-0612", etablissement:"H. Régional Garoua",ville:"Garoua",  patients:89,  consultations:1234, concordance:86, creeLE:"22/07/2024", valideLE:"25/07/2024", statut:"Actif", email:"barry@hrg.cm",    telephone:"+237 655 456 789", rangCommunaute:"#12/38",casPartages:"74 cas publiés",  activiteRecente:[{texte:"Consultation #89 enregistrée",quand:"Il y a 2j"}] },
];
...
function ModaleProfl({ medecin:m, onClose, onSuspendre, onSupprimer, darkMode }) {
  ...
  const infos=[
    ...
    {l:"Établissement",v:m.etablissement,cls:val},
            ...
            <div className={`flex items-center justify-between gap-4 p-4 rounded-xl border ${card}`}>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className={`text-sm ${lbl}`}>{m.specialite} · {m.etablissement}</p>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div className={`relative w-full max-w-md rounded-2xl shadow-2xl pointer-events-auto ${darkMode?"bg-gray-900 border border-gray-700":"bg-white border border-gray-200"}`}>
          ...
          <div className="px-6 py-5 space-y-3">
            <div className={`flex items-start gap-3 px-4 py-3 rounded-xl border ${darkMode?"bg-red-950/30 border-red-800/40 text-red-300":"bg-red-50 border-red-100 text-red-700"}`}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              <p className="text-sm leading-relaxed">Supprimer définitivement le compte de <span className="font-bold">{m.nom}</span> ? Toutes ses données seront effacées de la plateforme.</p>
            </div>
...
export default function MedecinsActifs({ darkMode: initialDarkMode }) {
  ...
  const handleExportCSV=useCallback(()=>{
    const h=["Nom","CNOM","Établissement","Ville","Patients","Consultations","Concord. IA","Créé le","Validé le"];
    const r=medecins.map(m=>[m.nom,m.cnom,m.etablissement,m.ville,m.patients,m.consultations,`${m.concordance}%`,m.creeLE,m.valideLE]);
  ...
  const handleSuppression=useCallback(()=>{
    ...
    showToast(`Compte de ${modaleSuppr.nom} supprimé définitivement`,"error"); setModaleSuppr(null);
  },[modaleSuppr]);
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${darkMode ? "bg-gray-950 text-white" : "bg-gray-50 text-gray-900"}`}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className={`text-xs uppercase tracking-wider border-b ${th} ${darkMode?"border-gray-700":"border-gray-200"}`}>
                {["Médecin","CNOM","Établissement","Ville","Patients","Consultations","Concord. IA","Créé le","Validé le","Statut","Actions"].map(h=>(
            ...
            <tbody className={`divide-y ${div}`}>
              {medecins.length===0?(
                <tr><td colSpan={11} className={`text-center py-16 ${muted}`}><div className="flex flex-col items-center gap-3"><svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="opacity-30"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg><span className="text-sm">Aucun médecin actif</span></div></td></tr>
              ):medecins.map(m=>(
                <tr key={m.id} className={`transition-colors duration-100 ${rh}`}>
                  ...
                  <td className={`px-4 py-3.5 text-xs ${muted}`}>{m.etablissement}</td>
      ...
      {medecins.length>0&&<p className={`text-xs mt-3 text-right ${muted}`}>Dernière mise à jour : {new Date().toLocaleString("fr-FR")}</p>}
